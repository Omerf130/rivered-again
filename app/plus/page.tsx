...
export default function PlusPage() {
  ...
  // Start editing a plus
  const handleEdit = (plus: Plus) => {
    setEditingId(plus._id);
    setEditFormData({
      name: plus.name,
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setError(null);
      const response = await fetch(`/api/pluses/${editingId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (result.ok) {
        ...
        setError(result.message || "Failed to update plus");
      }
    } catch (err: any) {
      setError(err.message || "An error occurred");
    }
  ...
  return (
    <>
      <h1 className="text-4xl font-bold text-center mb-8 text-black dark:text-zinc-50">
        ...
        {/* Edit Plus Form (shown when editing) */}
        {editingId && (
          <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-md p-6 mb-8 border-2 border-purple-500">
            <h2 className="text-2xl font-semibold mb-4 text-black dark:text-zinc-50">
              Edit Plus
            </h2>
